...
# Use a map to create the protection groups
resource "aws_shield_protection_group" "example" {
  for_each = var.resources

  count               = each.value ? 1 : 0
  protection_group_id = each.key
  aggregation         = lookup(var.aggregations, each.key, "AGGREGATE")
  pattern             = "BY_RESOURCE_TYPE"
  resource_type       = upper(underscore(each.key))
